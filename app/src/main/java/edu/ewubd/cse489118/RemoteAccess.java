...
public class RemoteAccess {
    ...
    public String makeHttpRequest(String url, String method, List<NameValuePair> params) {
        ...
        try {
            ...
            http.connect();
            is = http.getInputStream();//packet packet wise data
            //System.out.println("Here 4");
            BufferedReader reader = new BufferedReader(new InputStreamReader(is, "iso-8859-1"), 8);//store buffer
            StringBuilder sb = new StringBuilder();
            String line = null;
            //Buffer Read with every new line store
            while ((line = reader.readLine()) != null) {
                sb.append(line + "\n");
            }
            is.close();
            return sb.toString();//Return store data
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
